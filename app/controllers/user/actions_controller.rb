class User::ActionsController < User::Base
    ...
    def date
        if current_user.gender_cd == "男性" #ログインユーザが男性の場合
            @actions = Action.where.not(matching_at: nil).where(male_user_id: current_user.id).includes(:female_user)
        else #ログインユーザが女性の場合
            @actions = Action.where.not(matching_at: nil).where(female_user_id: current_user.id).includes(:male_user)
        end
    end
end
